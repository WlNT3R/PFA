...
/* Don't change this type. The function priceOption takes an argument of type
 * Option *.   */
typedef struct {
  ...
  /* Parameters of the distribution of random variable S_T (price of the
   * underlying asset at time T) */
  double mu;
  double sig;
} Option;

/* Don't change this type. The functions about insurance take an argument of
 * type InsuredClient *.  */
typedef struct {
  /* m and s are the parameters of the log-normal distribution of random
     variables X1 and X2 (the reimbursements at each claim) */
  double m;
  double s;

  /* An array of 3 elements. p[0], p[1] and p[2] are the probabilities that the
     client has 0, 1 or 2 claims during the year. */
  double *p;
} InsuredClient;
...
/* Initialize the integration variables.
   ...
   This functions sets the global variables pfaQF and pfa_dt to the values that
   you will use.
*/
extern bool init_integration(char *quadrature, double dt);

/* Normal distribution : density (phi) and cumulative distribution function
 * (PHI) */
